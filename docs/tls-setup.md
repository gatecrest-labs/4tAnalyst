...
> **Note:** When nginx is terminating TLS, set `allowed_hosts` to the bare hostname with no port — e.g. `["4tanalyst.example.com"]`, not `["4tanalyst.example.com:8000"]`. The `:port` form is only correct when engineers connect directly to uvicorn.
